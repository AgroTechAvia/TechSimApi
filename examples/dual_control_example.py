...
def main(args):
    ...
    client_1 = SimClient(address="127.0.0.1", port=8080)
    client_2 = SimClient(address="127.0.0.1", port=8081)

    HOST = args.inav_host

    PORT_1 = args.inav_port_1
    ADDRESS_1 = (HOST, PORT_1)

    PORT_2 = args.inav_port_2
    ADDRESS_2 = (HOST, PORT_2)

    tcp_transmitter_1 = TCPTransmitter(ADDRESS_1)
    tcp_transmitter_1.connect()
    control_1 = MultirotorControl(tcp_transmitter_1)

    tcp_transmitter_2 = TCPTransmitter(ADDRESS_2)
    tcp_transmitter_2.connect()
    control_2 = MultirotorControl(tcp_transmitter_2)

    print("Z/X Thortle \nQ/E Yaw \nW/S Pitch \nA/D Roll")

    time.sleep(1)

    control_1.send_RAW_RC([1500, 1500, 1000, 1500, 1000, 1000, 1000])
    control_1.receive_msg()

    control_2.send_RAW_RC([1500, 1500, 1000, 1500, 1000, 1000, 1000])
    control_2.receive_msg()

    time.sleep(0.5)

    control_1.send_RAW_RC([1500, 1500, 1000, 1500, 2000, 1000, 1000])
    control_1.receive_msg()

    control_2.send_RAW_RC([1500, 1500, 1000, 1500, 2000, 1000, 1000])
    control_2.receive_msg()

    time.sleep(0.5)

    control_1.send_RAW_RC([1500, 1600, 1350, 1500, 2000, 1000, 1000])
    control_1.receive_msg()

    control_2.send_RAW_RC([1500, 1600, 1350, 1500, 2000, 1000, 1000])
    control_2.receive_msg()

    time.sleep(1)

    control_1.send_RAW_RC([1500, 1500, 1320, 1500, 2000, 1000, 1000])
    control_1.receive_msg()

    control_2.send_RAW_RC([1500, 1500, 1320, 1500, 2000, 1000, 1000])
    control_2.receive_msg()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--inav_port_1', type=int, default=5762)
    parser.add_argument('--inav_port_2', type=int, default=5862)
