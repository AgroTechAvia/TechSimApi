...
def tracking_aruco():
    pid_pitch = PID(0.2, 0.0003, 0.2)
    pid_roll = PID(0.2, 0.0003, 0.2)
    distance = 1
    accuracy = 0.1
    ...
    while True:
        ...
        if errors:
            ...
            if abs(pitch_error) < accuracy and abs(roll_error) < accuracy and abs(yaw_error) < accuracy:
                client.setVelXYYaw(0, 0, 0)
                time.sleep(1)
...
def aruco_regulation(pitch_error, roll_error, yaw_error, pid_pitch:PID, pid_roll:PID):
    ...
    PID_yaw = constrain (yaw_error, 0.8)
