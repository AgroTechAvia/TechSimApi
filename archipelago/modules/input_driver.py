...
def on_press(key):
    ...
    try:
        if is_control == True:
            if key.char == 'w':
                rc_control[1] = min(rc_control[1] + 100, 2000)
                print(f'Increased Pitch control: {rc_control[1]}')

            elif key.char == 's':
                rc_control[1] = max(rc_control[1] - 100, 1000)
                print(f'Decreased Pitch control: {rc_control[1]}')

            elif key.char == 'd':
                rc_control[0] = min(rc_control[0] + 100, 2000)
                print(f'Increased Roll control: {rc_control[0]}')

            elif key.char == 'a':
                rc_control[0] = max(rc_control[0] - 100, 1000)
                print(f'Decreased Roll control: {rc_control[0]}')

            elif key.char == 'e':
                rc_control[3] = min(rc_control[3] + 100, 2000)
                print(f'Increased Yaw control: {rc_control[3]}')
                
            elif key.char == 'q':
                rc_control[3] = max(rc_control[3] - 100, 1000)
                print(f'Decreased Yaw control: {rc_control[3]}')

            elif key.char == 'x':
                rc_control[2] = min(rc_control[2] + 100, 2000)
                print(f'Increased Thortle control: {rc_control[2]}')

            elif key.char == 'z':
                rc_control[2] = max(rc_control[2] - 100, 1000)
